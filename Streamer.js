...
import { NativeModules, NativeEventEmitter, requireNativeComponent, View, Text } from 'react-native';
...
console.log('LarixStreamer', LarixStreamer);
...
export default class Streamer extends React.Component {
    ...
    render() {
        ...
        (<StreamerView style={{flex: 1,width:"100%", backgroundColor: "black"}}
            autoStart={true} 
            ...
            switch(status) {
                ...
                case 'unknownFail':
                    ...
                    if (info != null) {
                        ...
                        errorMmessage = `${name}: Unknown connection error.`
                    }
                    retry = true
                    break
                case 'authFail':
    ...
    onStreamerStats = (data) => {
        ...
        Object.entries(data).forEach (entry => {
            ...
            if (typeof stats == 'object') {
                ...
                statsStr += `${name}: ${bitrateStr}, ${trafficStr}`
            }
        })
    ...
    onCameraChanged = (info) => {
        ...
        if (info.error != null) {
            ...
            if (info.error == 'changeCameraFailed') {
              errorMsg = 'The selected video size or frame rate is not supported by the destination camera'
    ...
    onConnect = (connId) => {
        ...
        idList.forEach((id, idx) => {
            ...
            //console.log(config)
            let state = {state: 'connecting', name: config?.name ?? '', url: config?.url ?? ''}
            connectionState[id] = state
        })
        //console.log(connectionState)

        this.setState({configList: undefined, 
