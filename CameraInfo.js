...
class CameraInfo {
    ...
    getDefaultBackCamera = () => {
        if (this.backCams == null || this.backCams.length == 0) { return null }
    ...
    getDefaultFrontCamera = () => {
        if (this.frontCams == null || this.frontCams.length == 0) { return null }
    ...
    getResolutions = (camId) => {
        ...
        return sizes.filter((resStr) => {
            ...
            return (parseInt(w) <= CameraInfo.maxWidth && parseInt(h) <= CameraInfo.maxHeight)
        })
